...
def filter_data(df):
    try:
        ...
        sort = "ESTADO" #st.sidebar.selectbox("Sort column", cols, index=25)
        #unique_vals = df[sort].unique().to_list()
        by = "ASISTIDA" #st.sidebar.selectbox("Sort value", unique_vals, index=0)
        ...
        if st.sidebar.button("Filter", type="primary"):
            ...
            df.write_csv(f"../lostless_data/data/data.csv")
